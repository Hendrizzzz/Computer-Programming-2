...
public class FractionArithmetic {
    ...
    private void run() throws IOException {
        while(true){
            ...
            // if the user chose to reduce a fraction
                ...
                Fraction fraction = setUpFraction("the");
    ...
    private void displayResult(Fraction firstFraction, Fraction secondFraction, Fraction result, String operation, String term) {
        ...
        if (result.toString().contains("/") && result.getNumerator() > 0) {
            System.out.printf(GREEN + "%2s%9s%4s%.4f%5s%n", "║ =", result, " or ", result.toDouble(), "║");
        } else if (result.toString().contains("/")) {
            System.out.printf(GREEN + "%2s%9s%4s%.4f%4s%n", "║ =", result, " or ", result.toDouble(), "║");
        } else {
            System.out.printf(GREEN + "%3s%12s%12s%n", "║ =", result, "║");
        }
